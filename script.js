...
const backgroundElement = document.querySelector(".top");
...
function backgroundAnimation() {
    ...
    const x = mousePos[0] / backgroundElement.clientWidth * 100;
    const y = mousePos[1] / backgroundElement.clientHeight * 100;
    bgGradientPos[0] += (x - bgGradientPos[0]) / 20 * deltaTime;
    bgGradientPos[1] += (y - bgGradientPos[1]) / 20 * deltaTime;
    backgroundElement.style.background = `radial-gradient(circle at ${bgGradientPos[0]}% ${bgGradientPos[1]}%, #003, #001)`;
